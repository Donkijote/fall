...
describe("Moves", () => {
  ...
  it("should return state if player has not played card", () => {
    ...
    const newState = playCard(state, initialState.players[0].id, deck[0]);
    expect(newState).toBe(state);
  });
  ...
  it("should capture last table card, get clean table points", () => {
    ...
    expect(newState.scores.values[initialState.players[0].id]).toBe(4);
  });
  it("should capture last played card, get fall points", () => {
    ...
    const newState = playCard(state, initialState.players[0].id, deck[11]);
    expect(newState.table).toHaveLength(1);
    expect(newState.players[0].collected).toHaveLength(2);
    expect(newState.scores.values[initialState.players[0].id]).toBe(1);
  });
  it("should capture last 10 played card, get 2 fall points", () => {
    ...
    state.players[0].collected = [];
    const newState = playCard(state, initialState.players[0].id, deck[17]);
    expect(newState.table).toHaveLength(1);
    expect(newState.players[0].collected).toHaveLength(2);
    expect(newState.scores.values[initialState.players[0].id]).toBe(2);
  });
  it("should capture last 11 played card, get 3 fall points", () => {
    ...
    state.players[0].collected = [];
    const newState = playCard(state, initialState.players[0].id, deck[18]);
    expect(newState.table).toHaveLength(1);
    expect(newState.players[0].collected).toHaveLength(2);
    expect(newState.scores.values[initialState.players[0].id]).toBe(3);
  });
  it("should capture last 12 played card, get 4 fall points", () => {
    ...
    state.players[0].collected = [];
    const newState = playCard(state, initialState.players[0].id, deck[19]);
    expect(newState.table).toHaveLength(1);
    expect(newState.players[0].collected).toHaveLength(2);
    expect(newState.scores.values[initialState.players[0].id]).toBe(4);
  });
